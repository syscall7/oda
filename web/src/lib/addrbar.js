...
function Location(posX, vma, style) {
    ...
    /* The X pixel vma on the navigation bar */
    this.posX = posX;
 ...
 *                             C L A S S
 * ------------------------------------------------------------------ */
export function AddressNavBar(canvasId, sects, parcels) {
    ...
    function draw() {
        ...
        function drawTextArrayCentered(txtStyleArray, x, y, h, font) {
            ...
            let txt_joined = $.map(txtStyleArray, function(txtStyle) {
                return txtStyle.txt;
            }).join(SEPARATOR);
    ...
    function addUser(uid) {
        if (userArrows[uid] == undefined) {
            let getNewArrowStyle = function () {
                ...
                let colorIndex = numUsers % colors.length;
